      ...
      for (const { re, rec } of templateUnescaped) {
        for (const m of content.matchAll(re)) {
          ...
          vulns.push({
            ...
            file: file.path,
            line: lineFromIndex(content, idx),
            pattern: escapeForDisplay(m[0]),
            recommendation: escapeForDisplay(rec),
            severity: "high",
            confidence: 0.9,
            snippet: escapeForDisplay(
              content.slice(Math.max(0, idx - 50), idx + 50)
            ),
            sanitized: false,
          });
        }
        ...
        for (const m of content.matchAll(re)) {
          ...
          if (hasSource && (!sanitized || cssDanger)) {
            ...
            vulns.push({
              ...
              file: file.path,
              line: lineFromIndex(content, idx),
              pattern: escapeForDisplay(desc),
              recommendation: escapeForDisplay(recommendation),
              severity,
              confidence: sanitized ? 0.6 : 0.9,
              snippet: escapeForDisplay(
                content.slice(Math.max(0, idx - 100), idx + 100)
              ),
              sanitized,
            });
          }
